...
const Show_cluster = () => {
  const { delete_image_from_server, total_received_images, change_cluster, cropped_unique_images, suggestion_list, set_suggestion_list, current_cluster, unique_images, socket, fetched_cluster_info, set_fetched_cluster_info } = useContext(Sitecontext)
  const [position, set_position] = useState('-right-[500px]')
  const cluster_id_params = useParams()
  const Navigate = useNavigate()
  const [image_to_be_moved, set_image_to_be_moved] = useState("")
  ...
  const handle_suggestion_box = (img) => {

    if (img && position == '-right-[500px]') {
      set_image_to_be_moved(prev => img)
      set_position('right-0')
    }
    else {
      setTimeout(() => {
        set_image_to_be_moved("")
      }, 1000);
  ...
  useEffect(() => {
    if (!ref.current) {
      ...
      socket.on("cluster_id_refreshed", (clustered_data) => {
        // console.log("Hlo")
        if (Object.keys(clustered_data?.data).length != 0) {
          ...
          let filtered_matched_score = dataArray.filter((item) => item?.matchedScore < 80)  // score less than 100
          // filtered_matched_score.shift()
          if (filtered_matched_score.length != 0) {
            filtered_matched_score = filtered_matched_score.map((item) => item.file_id)
            set_suggestion_list(prev => ({ ...prev, [clustered_data?.data?.clusterId]: [...filtered_matched_score] }))
          }
          ...
          Navigate("/cluster")
          set_fetched_cluster_info(prevState => {
            // Create a new copy of prevState
            const newState = { ...prevState };
            // Use the delete operator to remove the key
            delete newState[cluster_id_params.clusterId];
            return newState;
          })
  ...
  return (
    <>
      <div className='fixed w-full z-20'>
        ...
        <div className=' bg-white px-2 sm:px-5 flex h-30 sm:h-36 w-full'>
          ...
          <div className='w-1/4 sm:w-4/12 flex items-center relative'>
            <img src={cropped_unique_images[current_unique_file_id]} className='h-20 w-20 sm:h-32 sm:w-32 rounded-full' />
            
          </div>

          {
            suggestion_list.hasOwnProperty(cluster_id_params.clusterId) ?
              suggestion_list[cluster_id_params.clusterId].length != 0 ?
                  ...
                  <div className='h-5/6 sm:h-full xl:w-4/6'>
                    <div className="flex p-1 sm:gap-3 h-full bg-white border border-gray-300 rounded-xl overflow-hidden items-center justify-start">
                      <div className="relative w-1/4 lg:w-1/5 h-full">
                        {
                          total_received_images[suggestion_list[cluster_id_params.clusterId][0]] ?
                            <img className="w-full h-full rounded-md" loading="lazy" src={total_received_images[suggestion_list[cluster_id_params.clusterId][0]]} /> :
                            <img className="w-full h-full rounded-md bg-gray-300 animate-pulse" loading="lazy" />
                        }
                      </div>
                      <div className="flex flex-col w-3/4 gap-2 px-2 py-2">
                        ...
                        <div className='flex items-center'>
                          ...
                          <button onClick={() => handle_suggestion_box(total_received_images[suggestion_list[cluster_id_params.clusterId][0]])} type="button" className="text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-1 text-center inline-flex items-center me-2 ">
      ...
      <div className={`fixed z-30 mt-[24.5vh] pt-3 md:pt-6 px-3 bg-white rounded-lg shadow-2xl ${position} transition-all duration-700 top-0 mb-[7vh] w-2/3 sm:w-[50%] lg:w-1/3 xl:w-1/4 h-[67vh] border-2 border-gray-300`}>
        <div className='flex justify-between h-1/5 items-center'>
          <img className='h-24 w-24 lg:h-28 lg:w-28 rounded-lg border-2 border-gray-300' src={image_to_be_moved} />
              ...
              return (
                current_cluster != element.clusterId ?
                  <div key={index} className='h-14 w-14 cursor-pointer'>
                    {
                      total_received_images[element.fileId] ?
                        <img onClick={()=>{
                          setTimeout(() => {
                            set_image_to_be_moved("")
                          }, 1000);
                          set_position('-right-[500px]')
                          change_cluster(cluster_id_params.clusterId,suggestion_list[cluster_id_params.clusterId][0],element.clusterId)}} className='h-full w-full rounded-full' src={`${cropped_unique_images[element.fileId]}`}/>:
