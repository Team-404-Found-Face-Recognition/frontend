...
const SOCKET_URL = 'https://digiboxx.shubhamistic.com/data';
...
const All_context = (props) => {
  ...
  const [cropped_unique_images, set_cropped_unique_images] = useState({})
  const [suggestion_list,set_suggestion_list]=useState({})
  ...
  const cropImage = async (unique_images_data) => {
    for (let index of unique_images_data.data) {
      if (!cropped_unique_images.hasOwnProperty(index.fileId)) 
      {
        try {
          const response=await fetch("https://digiboxx.shubhamistic.com/cluster/crop",{
            method : "POST",
            headers : {
          ...
          const img_url=URL.createObjectURL(image)
          set_cropped_unique_images((prev)=>({...prev,[index.fileId]:img_url}))
        }
        catch(error)
        {
           // Do nothing 
  ...
  const change_name_unique_image = async (cluster_id, cluster_name) => {
    try {
      const response = await fetch("https://digiboxx.shubhamistic.com/cluster/change-name", {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `${access_token}`
  ...
  const updated_images_fetch = async () => {
    for (let index of sorted_data.data[0].info) {
      if (!total_received_images[index.fileId]) {
        try {
          const response = await fetch("https://digiboxx.shubhamistic.com/data/get", {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `${access_token}`
  ...
  const login_request = async (email, password) => {
    try {
      const response = await fetch(`https://digiboxx.shubhamistic.com/auth/sign-in`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const signup_request = async (email, password) => {
    try {
      const response = await fetch(`https://digiboxx.shubhamistic.com/auth/sign-up`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const change_cluster = async (cluster_id, file_id, new_cluster_id) => {
    // console.log(cluster_id,file_id,new_cluster_id)
    try {
      const response = await fetch("https://digiboxx.shubhamistic.com/cluster/user-feedback", {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `${access_token}`
      ...
      const json = await response.json()
      if (response.ok) {
        toast.success("Successfully Changed Clustered")
      }
      else {
  ...
  const delete_image_from_server = async (file_id) => {
    try {
      const response = await fetch("https://digiboxx.shubhamistic.com/data/delete", {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `${access_token}`
  ...
  const upload_live_server = async (file) => {
    ...
    try {
      const response = await axios.post('https://digiboxx.shubhamistic.com/data/upload', formData, {
        headers: {
          'Authorization': `${access_token}`
        },
      });
  ...
  useEffect(() => {
    const authenticate = async () => {
      try {
        ...
        if (refresh_token) {
          const response = await fetch(`https://digiboxx.shubhamistic.com/auth/authenticate`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  return (
    <Sitecontext.Provider value={{ isChecked, total_received_images,cropped_unique_images, suggestion_list,set_suggestion_list, fetched_cluster_info, logged_in, unique_images, set_current_cluster, current_cluster, sorted_data, uid, access_token, uploadProgress, total_file_size, set_total_file_size, current_file_size, set_current_file_size, uploadedFiles, failedFiles, setFailedFiles, setUploadProgress, setIsChecked, delete_image_from_server, signup_request, signout, change_name_unique_image, login_request, upload_live_server, set_fetched_cluster_info, change_cluster, socket: socketRef.current }}>
      {props.children}
